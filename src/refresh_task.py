...
class RefreshTask:
        ...
        while True:
            try:
                with self.condition:
                    ...
                    if self.display_manager.is_asleep():
                        logger.debug("Display asleep; skipping refresh tick")
                        continue
    ...
    def _determine_next_plugin(self, playlist_manager, latest_refresh_info, current_dt):
        ...
        if not should_refresh:
            latest_refresh_str = latest_refresh_dt.strftime(
                '%Y-%m-%d %H:%M:%S') if latest_refresh_dt else "None"
            logger.info(
...
class RefreshAction:
    ...
    def refresh(self, plugin, device_config, current_dt):
        ...
        raise NotImplementedError(
            "Subclasses must implement the refresh method.")
    ...
    def get_plugin_id(self):
        ...
        raise NotImplementedError(
            "Subclasses must implement the get_plugin_id method.")
    ...
    def execute(self, plugin, device_config, current_dt: datetime):
        ...
        if self.plugin_instance.should_refresh(current_dt) or self.force:
            ...
            image = plugin.generate_image(
                self.plugin_instance.settings, device_config)
